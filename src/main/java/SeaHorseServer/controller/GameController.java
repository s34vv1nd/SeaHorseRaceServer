...
public class GameController {
  ...
  private void launch() throws IOException {
    if (checkBasicConditions() && GameService.launch(thread.getCurrentUser())) {
      RoomService.sendToRoom(thread.getCurrentUser().getRoomId(),
          "GAME launch success " + thread.getCurrentUser().getColor());
    } else {
  ...
  private void move() throws IOException {
    if (checkBasicConditions()) {
      ...
      if (result != -1) {
        RoomService.sendToRoom(thread.getCurrentUser().getRoomId(),
            "GAME move success " + thread.getCurrentUser().getColor() + " " + startPos + " " + result);
      } else {
  ...
  private void exit() throws IOException {
    if (checkBasicConditions()) {
      ...
      if (GameService.exit(thread.getCurrentUser())) {
        thread.send("GAME exit success " + thread.getCurrentUser().getUsername());
        RoomService.sendToRoom(roomId, "GAME exit success " + thread.getCurrentUser().getUsername());
      }
    } else {
